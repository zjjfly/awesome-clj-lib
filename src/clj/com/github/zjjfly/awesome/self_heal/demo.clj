...
(ns com.github.zjjfly.awesome.self-heal.demo
  (:require [clojure.spec.alpha :as s]
            [clojure.spec.gen.alpha :as gen]
            [clojure.string :as string]
            [com.github.zjjfly.awesome.self-heal.candidates]))
...
(defn failing-function-name [e]
  (as-> (.getStackTrace e) ?
        (map #(.getClassName %) ?)
        (filter #(string/starts-with? % "com.github.zjjfly.awesome.self_heal.demo") ?)
        (first ?)
        (string/split ? #"\$")
        (last ?)
        (string/replace ? #"_" "-")
...
(defn find-spec-candidate-match [fname fspec-data failing-input]
  (let [candidates (->> (s/registry)
                        keys
                        (filter #(string/starts-with? (namespace %) "com.github.zjjfly.awesome.self-heal.candidates"))
                        (filter symbol?))]
...
(with-healing (report []))
;ERROR in function com.github.zjjfly.awesome.self-heal.demo/calc-average Divide by zero -- looking for replacement
;Retriving spec information for function  {:args :com.github.zjjfly.awesome.self-heal.demo/cleaned-earnings, :ret :com.github.zjjfly.awesome.self-heal.demo/average, :fn nil}
;Checking candidates  (com.github.zjjfly.awesome.self-heal.candidates/better-calc-average)
;----------
;**Looking at candidate  com.github.zjjfly.awesome.self-heal.candidates/better-calc-average
;****Comparing args :com.github.zjjfly.awesome.self-heal.candidates/numbers :com.github.zjjfly.awesome.self-heal.demo/cleaned-earnings with input [[]]
;****Comparing seed  [[1 2 3 4 5]] with new function
;****Result: old 3 new 3
;Found a matching candidate replacement for failing function com.github.zjjfly.awesome.self-heal.demo/calc-average  for input []
;Replacing with candidate match com.github.zjjfly.awesome.self-heal.candidates/better-calc-average
;----------
